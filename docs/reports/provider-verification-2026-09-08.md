...
- El detalle canónico consulta también `images.logos` y prefiere el logotipo español (después inglés/neutro) para la cabecera; el PNG se mantiene aislado del póster de las tarjetas y el título textual queda disponible para accesibilidad.
...
- El reparador de identidades usa cruces TMDB→Wikidata y MAL→AniList exactos antes de búsquedas difusas. Después de los lotes iniciales de 20 y 50 filas, se aplicaron cuatro lotes continuables de 100, 500, 500 y 500 filas: 27, 132, 357 y 264 filas recibieron identidad; 103 filas adicionales conservaron AniList aunque su MAL ya estaba usado por otra ficha. Los pendientes y conflictos se conservaron sin conjeturas. La ejecución es seca por defecto, admite `--after-id` y devuelve `next_after_id` para reanudarla.
