...
import { bentoBackdropUrl, cardPosterUrl } from '../utils/imageSizes';
...
export const BentoCollection: React.FC<BentoCollectionProps> = ({
  ...
  return (
    <section className="space-y-4" style={{ contentVisibility: 'auto', containIntrinsicSize: '0 460px' }}>
      {/* SECTION HEADER */}
...
const BentoMainCard: React.FC<BentoCardProps> = ({ media, onSelect, onHover }) => {
  ...
  const image = bentoBackdropUrl(media);
...
const BentoMiniCard: React.FC<BentoCardProps> = ({ media, onSelect, onHover }) => {
  const image = cardPosterUrl(media);
    ...
    >
      ...
      <div className="relative z-10 space-y-1">
        <div className="flex items-center justify-between text-[10px] text-zinc-400 font-mono">
          <span className="capitalize">{contentLabel(media.category)}</span>
          {media.rating && (
