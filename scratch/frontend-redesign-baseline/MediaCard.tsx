...
export const MediaCard: React.FC<MediaCardProps> = React.memo(({
  ...
  const poster = cardPosterUrl(media);
  const cleanGenres = cleanDisplayGenres(media.genres);
  const primaryGenre = cleanGenres[0] || (media.category ? contentLabel(media.category) : 'Contenido');
  const displayTitle = cleanDisplayTitle(media.title);
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[2/3] w-full overflow-hidden rounded-xl bg-zinc-900 border border-zinc-800 shadow-sm transition-[border-color] duration-300 group-hover/card:border-zinc-700">
        {poster ? (
          <SmartImage
            src={poster}
            alt={displayTitle}
            sizes="(min-width: 1280px) 192px, (min-width: 640px) 160px, 44vw"
      ...
      <div className="mt-2 px-0.5 space-y-0.5">
        <h4 className="font-display text-sm font-semibold text-zinc-100 group-hover/card:text-amber-400 truncate transition-colors duration-150" title={displayTitle}>
          {displayTitle}
        </h4>
        <div className="flex items-center gap-1.5 text-xs text-zinc-400">
          <span className="text-zinc-400">{primaryGenre}</span>
          {media.year && (
            <>
