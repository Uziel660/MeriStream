...
// Development/test sessions use a process-local secret when no environment
// value exists. Production fails closed above instead of accepting a known key.
export const JWT_SECRET = configuredJwtSecret || randomBytes(32).toString("hex");
const authRouter = Router();

export function verifyAuthToken(token: string): { id: string; username: string; is_admin?: boolean } | null {
  try {
    return jwt.verify(token, JWT_SECRET) as { id: string; username: string; is_admin?: boolean };
  } catch {
    return null;
  }
}
...
authRouter.post("/register", async (req: Request, res: Response) => {
  try {
    ...
    const token = jwt.sign(
      { id: newUser.id, username: newUser.username, is_admin: Boolean(newUser.is_admin) },
      JWT_SECRET,
      { expiresIn: "30d" }
    );
