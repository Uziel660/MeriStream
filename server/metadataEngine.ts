...
export interface EnrichedMetadata {
  ...
  tmdb_id?: number;
  /** Identificador canónico de IMDb (formato "tt1234567"). */
  imdb_id?: string | null;
  /** Identificador de TheTVDB para series. */
  tvdb_id?: number | null;
...
export function isValidImdbId(id: unknown): id is string {
  return typeof id === "string" && /^tt\d{5,12}$/i.test(id.trim());
}

export function isValidTvdbId(id: unknown): id is number {
  return typeof id === "number" && Number.isInteger(id) && id > 0;
}

export function extractExternalIds(details: TmdbItemDetails | null | undefined): TmdbExternalIds {
  if (!details) return { imdb_id: null, tvdb_id: null };

  let imdb_id: string | null = null;
  const rawImdb = details.external_ids?.imdb_id || details.imdb_id;
  if (isValidImdbId(rawImdb)) {
    imdb_id = rawImdb.trim().toLowerCase();
  }

  let tvdb_id: number | null = null;
  const rawTvdb = details.external_ids?.tvdb_id;
  if (isValidTvdbId(rawTvdb)) {
    tvdb_id = rawTvdb;
  } else if (typeof rawTvdb === "string" && /^\d+$/.test(rawTvdb.trim())) {
    const parsed = Number.parseInt(rawTvdb.trim(), 10);
    if (isValidTvdbId(parsed)) tvdb_id = parsed;
  }

  return { imdb_id, tvdb_id };
}

const TMDB_DETAILS_TTL_MS = 60 * 60 * 1000; // 1 hora
const tmdbDetailsMemoryCache = new Map<string, { data: TmdbItemDetails; fetchedAt: number }>();

/**
 * Consulta la ficha completa de una película o serie en TMDB con append_to_response=external_ids.
 * Cacheado en memoria con TTL de 1 hora y protegido por el limitador global acquireTmdbRequestSlot.
 */
export async function fetchTMDBExternalDetails(
  mediaType: "movie" | "tv",
  tmdbId: number,
  language = "es-MX",
  ...
  const cached = tmdbDetailsMemoryCache.get(cacheKey);
  if (cached && Date.now() - cached.fetchedAt < TMDB_DETAILS_TTL_MS) {
    return cached.data;
  }
  ...
  try {
    ...
    const url = `https://api.themoviedb.org/3/${mediaType}/${tmdbId}?append_to_response=external_ids&language=${language}&api_key=${apiKey}`;
    const res = await fetch(url, { signal: controller.signal });
    clearTimeout(timer);

    if (res.ok) {
      const data = (await res.json()) as TmdbItemDetails;
      tmdbDetailsMemoryCache.set(cacheKey, { data, fetchedAt: Date.now() });
      return data;
    }
  } catch {
    // Falla controlada: retorna null permitiendo fallback transparente
  }
  return null;
}

/**
 * Búsqueda directa y determinista en TMDB por identificador exacto:
 * - Si identifier es numérico: consulta /movie/{id} o /tv/{id}.
 * - Si identifier es IMDb ("tt..."): consulta /find/{imdb_id}?external_source=imdb_id.
 */
export async function fetchTMDBById(
  ...
  const isImdb = isValidImdbId(rawId);
  const isNumeric = /^\d+$/.test(rawId);

  if (!isImdb && !isNumeric) return null;
  ...
  if (isImdb) {
    try {
      ...
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 4500);
      const url = `https://api.themoviedb.org/3/find/${encodeURIComponent(rawId)}?external_source=imdb_id&language=${language}&api_key=${apiKey}`;
      const res = await fetch(url, { signal: controller.signal });
      clearTimeout(timer);

      if (res.ok) {
        const findData = (await res.json()) as {
        ...
        if (kind === "movie" && movieMatch) {
          targetId = movieMatch.id;
          mediaType = "movie";
        } else if ((kind === "series" || kind === "anime") && tvMatch) {
          targetId = tvMatch.id;
          mediaType = "tv";
        } else if (movieMatch) {
          targetId = movieMatch.id;
        ...
        if (targetId) {
          return fetchTMDBMetadataById(mediaType, targetId, kind, language);
        }
      }
    } catch {
      return null;
  ...
  // Búsqueda por ID numérico de TMDB
  const numericId = Number.parseInt(rawId, 10);
  if (kind === "movie") {
    return fetchTMDBMetadataById("movie", numericId, kind, language);
  }
  if (kind === "series" || kind === "anime") {
    return fetchTMDBMetadataById("tv", numericId, kind, language);
  }

  // Probar movie luego tv si no hay kind
  const movieMeta = await fetchTMDBMetadataById("movie", numericId, "movie", language);
  if (movieMeta) return movieMeta;
  return fetchTMDBMetadataById("tv", numericId, "series", language);
}

async function fetchTMDBMetadataById(
  mediaType: "movie" | "tv",
  tmdbId: number,
  kind?: ContentKind,
  language = "es-MX",
): Promise<EnrichedMetadata | null> {
  const details = await fetchTMDBExternalDetails(mediaType, tmdbId, language);
  if (!details) return null;

  const externalIds = extractExternalIds(details);
  const isTV = mediaType === "tv";
  const title = details.title || details.name || "";
  const originalTitle = details.original_title || details.original_name || title;

  let overviewRaw = (details.overview || "").trim();
  let englishTitle: string | null = details.original_language === "en" ? originalTitle : null;

  if (!overviewRaw || !englishTitle) {
    const enDetails = await fetchTMDBExternalDetails(mediaType, tmdbId, "en-US");
    if (!overviewRaw && enDetails?.overview) overviewRaw = enDetails.overview.trim();
    if (!englishTitle) englishTitle = enDetails?.title || enDetails?.name || null;
  }
  ...
  const yearStr = details.release_date || details.first_air_date || "";
  const year = yearStr ? Number.parseInt(yearStr.substring(0, 4), 10) : 0;
  const rating = details.vote_average ? Math.round(details.vote_average * 10) / 10 : 8.0;

  let genres: string[] = [];
  if (Array.isArray(details.genres) && details.genres.length > 0) {
    genres = translateGenresToEs(details.genres.map((g) => g.name));
  }
  if (genres.length === 0) {
    genres = [isTV ? "Serie de TV" : "Película"];
  }

  const contentType: ContentKind = kind || (isTV ? "series" : "movie");
  ...
  return {
    ...
    rating,
    year,
    status: details.status === "Returning Series" ? "En emisión" : "Finalizado",
    genres,
...
export async function fetchTMDBMetadata(
  ...
  try {
    const fetchSearchMulti = async (lang: string) => {
      await acquireTmdbRequestSlot();
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), 4500);
      try {
        const res = await fetch(`https://api.themoviedb.org/3/search/multi?query=${encodeURIComponent(query)}&language=${lang}&api_key=${apiKey}`, {
          signal: controller.signal,
        });
        clearTimeout(timer);
        if (res.ok) {
    ...
    const candidateScore = (candidate: any, lang: string): number => {
      ...
      if (lang === "es-MX") score += 0.04;
      else if (lang === "es-ES") score += 0.02;

      if (isSubstantiveDescription(candidate?.overview)) score += 0.03;

      score += Math.min(0.03, Number(candidate?.popularity || 0) / 10000);
      return score;
    };

    const filterCandidates = (rawList: any[]) => {
      let filtered = rawList.filter((r: any) => r && r.media_type !== "person");
      if (kind === "movie") {
        filtered = filtered.filter((r: any) => r.media_type === "movie");
      } else if (kind === "series") {
        filtered = filtered.filter((r: any) => r.media_type === "tv");
      } else if (kind === "anime") {
        filtered = filtered.filter((r: any) => r.media_type === "tv" || r.media_type === "movie");
      }
      return filtered;
    };

    const initialResults = await fetchSearchMulti(language);
    const initialCandidates = filterCandidates(initialResults);
    let scoredList = initialCandidates.map((c: any, index: number) => ({
      candidate: c,
      index,
      score: candidateScore(c, language),
      lang: language,
    })).sort((a: any, b: any) => b.score - a.score || a.index - b.index);

    // Short-circuit check for multi-language cascade
    if (!identityOnly && language === "es-MX") {
      const topMatch = scoredList[0];
      const resultYear = Number.parseInt(String(topMatch?.candidate?.release_date || topMatch?.candidate?.first_air_date || "").slice(0, 4), 10);
      const yearMatches = !yearHint || resultYear === yearHint;
      const canShortCircuit = topMatch && topMatch.score >= 0.88 && isSubstantiveDescription(topMatch.candidate?.overview) && yearMatches;

      if (!canShortCircuit) {
        // Step 2: Try es-ES
        const esEsResults = await fetchSearchMulti("es-ES");
        const esEsCandidates = filterCandidates(esEsResults);
        const scoredEsEs = esEsCandidates.map((c: any, index: number) => ({
          candidate: c,
          index,
          score: candidateScore(c, "es-ES"),
          lang: "es-ES",
        }));

        // Pool and deduplicate
        const candidateMap = new Map<string, { candidate: any; index: number; score: number; lang: string }>();
        for (const item of [...scoredList, ...scoredEsEs]) {
          const key = `${item.candidate.media_type}:${item.candidate.id}`;
          const existing = candidateMap.get(key);
          if (!existing || item.score > existing.score) {
            candidateMap.set(key, item);
          }
        }
        scoredList = [...candidateMap.values()].sort((a, b) => b.score - a.score || a.index - b.index);

        // Step 3: Try en-US if top score is still < 0.80
        if (!scoredList[0] || scoredList[0].score < 0.80) {
          const enUsResults = await fetchSearchMulti("en-US");
          const enUsCandidates = filterCandidates(enUsResults);
          const scoredEnUs = enUsCandidates.map((c: any, index: number) => ({
            candidate: c,
            index,
            score: candidateScore(c, "en-US"),
            lang: "en-US",
          }));
          for (const item of scoredEnUs) {
            const key = `${item.candidate.media_type}:${item.candidate.id}`;
            const existing = candidateMap.get(key);
            if (!existing || item.score > existing.score) {
              candidateMap.set(key, item);
    ...
    let bestResult = scoredList[0]?.candidate;
    if (scoredList[0] && scoredList[0].score < 0.15) bestResult = undefined;

    if (yearHint != null && scoredList.length > 0) {
      const yearMatch = scoredList.find(({ candidate, score }: { candidate: any; score: number }) => {
        if (score < 0.15) return false;
        const y = Number.parseInt(String(candidate.release_date || candidate.first_air_date || "").substring(0, 4), 10);
        return y === yearHint;
      })?.candidate;
    ...
    if (bestResult && !(bestResult.overview || "").trim()) {
      const withOverview = scoredList
        .filter((entry) => (entry.candidate.overview || "").trim().length > 0 && entry.candidate.media_type === bestResult.media_type)
    ...
    if (identityOnly) {
      ...
      const identityResult = {
        ...
        __tmdb_match_score: scoredList.find((entry) => entry.candidate === bestResult)?.score || 0,
    ...
    const mediaTypeStr: "movie" | "tv" = isTV ? "tv" : "movie";

    // Fetch details & external IDs
    const details = typeof bestResult.id === "number"
      ? await fetchTMDBExternalDetails(mediaTypeStr, bestResult.id, "es-MX")
      : null;
    const externalIds = extractExternalIds(details);

    const title = details?.title || details?.name || bestResult.title || bestResult.name || query;
    const originalTitle = details?.original_title || details?.original_name || bestResult.original_title || bestResult.original_name || title;
    ...
    if (bestResult.original_language === "en" && (bestResult.overview || "").trim()) {
      ...
      englishTitle = enRes?.title || originalTitle || null;
      enOverview = enRes?.overview || "";
    }

    let overviewRaw: string = details?.overview?.trim() || bestResult.overview?.trim() || "";
    if (!overviewRaw && enOverview.trim()) overviewRaw = enOverview;
    const overview = await cleanAndTranslateDescription(overviewRaw);

    const posterPath: string | null = details?.poster_path || bestResult.poster_path || null;
    const backdropPath: string | null = details?.backdrop_path || bestResult.backdrop_path || null;
    const poster = posterPath ? `https://image.tmdb.org/t/p/w780${posterPath}` : null;
    const banner = backdropPath ? `https://image.tmdb.org/t/p/w1280${backdropPath}` : poster;

    const yearStr = details?.release_date || details?.first_air_date || bestResult.release_date || bestResult.first_air_date || "";
    const year = yearStr ? Number.parseInt(yearStr.substring(0, 4), 10) : 0;
    const rawVote = details?.vote_average ?? bestResult.vote_average;
    const rating = rawVote ? Math.round(Number(rawVote) * 10) / 10 : 8.0;
    ...
    if (genres.length === 0) {
      const genreMap = await fetchTMDBGenreMap(mediaTypeStr);
    ...
    if (isTV && (bestResult.origin_country?.includes("JP") || details?.original_language === "ja")) {
      contentType = "anime";
    }
