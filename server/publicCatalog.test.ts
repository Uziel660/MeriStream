...
describe("TMDB public catalog", () => {
  ...
  describe("parseUniversalCatalogQuery", () => {
    it("parses raw IMDb IDs and prefixed IMDb queries", () => {
      const q1 = parseUniversalCatalogQuery("tt1457767");
      expect(q1.type).toBe("imdb");
      expect(q1.imdbId).toBe("tt1457767");

      const q2 = parseUniversalCatalogQuery("imdb:tt1457767");
      expect(q2.type).toBe("imdb");
      expect(q2.imdbId).toBe("tt1457767");

      const q3 = parseUniversalCatalogQuery("IMDB-tt0133093");
      expect(q3.type).toBe("imdb");
      expect(q3.imdbId).toBe("tt0133093");
    });

    it("parses raw TMDB IDs and prefixed TMDB queries", () => {
      const q1 = parseUniversalCatalogQuery("138843");
      expect(q1.type).toBe("tmdb");
      expect(q1.tmdbId).toBe(138843);

      const q2 = parseUniversalCatalogQuery("tmdb:138843");
      expect(q2.type).toBe("tmdb");
      expect(q2.tmdbId).toBe(138843);

      const q3 = parseUniversalCatalogQuery("TMDB-603");
      expect(q3.type).toBe("tmdb");
      expect(q3.tmdbId).toBe(603);
    });

    it("parses TVDB prefixed queries", () => {
      const q1 = parseUniversalCatalogQuery("tvdb:81189");
      expect(q1.type).toBe("tvdb");
      expect(q1.tvdbId).toBe(81189);

      const q2 = parseUniversalCatalogQuery("tvdb-81189");
      expect(q2.type).toBe("tvdb");
      expect(q2.tvdbId).toBe(81189);
    });

    it("extracts embedded release year from title brackets and trailing years", () => {
      const q1 = parseUniversalCatalogQuery("El conjuro (2013)");
      expect(q1.type).toBe("title");
      expect(q1.title).toBe("El conjuro");
      expect(q1.year).toBe(2013);

      const q2 = parseUniversalCatalogQuery("The Matrix [1999]");
      expect(q2.type).toBe("title");
      expect(q2.title).toBe("The Matrix");
      expect(q2.year).toBe(1999);

      const q3 = parseUniversalCatalogQuery("Breaking Bad 2008");
      expect(q3.type).toBe("title");
      expect(q3.title).toBe("Breaking Bad");
      expect(q3.year).toBe(2008);

      const q4 = parseUniversalCatalogQuery("Inception", 2010);
      expect(q4.type).toBe("title");
      expect(q4.title).toBe("Inception");
      expect(q4.year).toBe(2010);
    });
  });

  describe("candidateToPublicCatalogShow", () => {
    it("correctly maps CatalogLookupCandidate to PublicCatalogShow", () => {
      const show = candidateToPublicCatalogShow({
        tmdb_id: 138843,
        imdb_id: "tt1457767",
        title: "El conjuro",
        ...
        backdrop_path: "/bntk45M2nE0w4K2l7.jpg",
        overview: "Ed and Lorraine Warren investigation.",
        vote_average: 7.5,
        genres: ["Terror"],
        confidence: 1.0,
      });

      expect(show.id).toBe("tmdb-movie-138843");
      expect(show.title).toBe("El conjuro");
      expect(show.tmdb_id).toBe(138843);
      expect(show.imdb_id).toBe("tt1457767");
      expect(show.kind).toBe("movie");
      expect(show.year).toBe(2013);
      expect(show.genres).toEqual(["Terror"]);
    });
  });

  describe("searchPublicCatalogExternal", () => {
    it("handles exact numeric TMDB queries and returns Interface Contract 1 candidates", async () => {
      process.env.TMDB_API_KEY = "test-key";
      vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
        const url = String(input);
        if (url.includes("/movie/138843")) {
          return new Response(JSON.stringify({
            id: 138843,
            title: "El conjuro",
            original_title: "The Conjuring",
            release_date: "2013-07-19",
            overview: "Terror investigation.",
        ...
        if (url.includes("/tv/138843")) {
          return new Response("not found", { status: 404 });
        }
        return new Response("not found", { status: 404 });
      }));

      const parsed = parseUniversalCatalogQuery("138843");
      const result = await searchPublicCatalogExternal(parsed, { limit: 10 }, "test-key");

      expect(result.ok).toBe(true);
      expect(result.exact_match).toBe(true);
      expect(result.candidates).toHaveLength(1);
      expect(result.candidates[0].tmdb_id).toBe(138843);
      expect(result.candidates[0].imdb_id).toBe("tt1457767");
      expect(result.candidates[0].media_type).toBe("movie");
      expect(result.candidates[0].confidence).toBe(1.0);
    });

    it("handles IMDb ID query via /find and returns exact match", async () => {
      process.env.TMDB_API_KEY = "test-key";
      vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL) => {
        const url = String(input);
        if (url.includes("/find/tt1457767")) {
          return new Response(JSON.stringify({
            movie_results: [
              {
                id: 138843,
      ...
      const parsed = parseUniversalCatalogQuery("tt1457767");
      const result = await searchPublicCatalogExternal(parsed, { limit: 10 }, "test-key");
