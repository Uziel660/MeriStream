...
import { detectAntiBot, recordAntiBotHit } from "../utils/antiBot";
  ...
  protected async fetchHtml(url: string, timeoutMs: number = 7500): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const verdict = detectAntiBot(response.status, response.headers, text.slice(0, 500), host);
        if (verdict.blocked) {
      ...
      if (verdict.blocked) {
        // Página de challenge servida con 200: no es contenido útil, no la devolvemos.
        recordAntiBotHit(host, verdict);
        console.warn(`[AntiBot] ${host}: ${verdict.kind} (${verdict.evidence}) [${url}]`);
        return null;
      }
      ...
      // En catch (timeout abort / error de conexión) el status HTTP real no es
      // observable con fetch estándar: sin status no hay veredicto fiable.
      return null;
    }
  }
