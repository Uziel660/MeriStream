...
describe("Video Stream Extraction & Resolvers", () => {
  // La lista negra del usuario (voe/mixdrop/filemoon) se filtra en el validador:
  // ya NO se aprueban como hosts conocidos ni deben sobrevivir a validateUrls.
  it("validates known embed hosts and drops blacklisted ones", async () => {
    const embeds = [
  ...
  it("drops voe/mixdrop/filemoon per user blacklist", async () => {
    const validated = await MediaValidator.validateUrls([
      "https://filemoon.sx/e/67890",
      "https://voe.sx/e/abcdef",
      "https://www.yourupload.com/embed/sample2",
    ]);
    expect(validated).toEqual(["https://www.yourupload.com/embed/sample2"]);
  });
