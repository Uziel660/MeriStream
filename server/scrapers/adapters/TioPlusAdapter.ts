...
export class TioPlusAdapter extends BaseScraperAdapter {
  ...
  public extractMetadata(
    ...
    const yearMatch =
      html.match(/href="[^"]*\/year\/(\d{4})"/i)?.[1] ||
      title.match(/\((\d{4})\)/)?.[1] ||
      description.match(/\b(19|20)\d{2}\b/)?.[0] ||
      url.match(/-(19\d{2}|20\d{2})(?:\/|$|\?)/i)?.[1]; // fallback: año al final del slug
    // Defecto #16/#24: si la ficha no declara año, devolver 0 (desconocido) y NO
    // el año corriente: showService decidirá con el enriquecimiento o el default.
