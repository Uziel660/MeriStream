...
export const api = {
  ...
  async getAdminOperations(): Promise<{ definitions: AdminOperationDefinition[]; operations: AdminOperationStatus[] }> {
    return request<{ definitions: AdminOperationDefinition[]; operations: AdminOperationStatus[] }>("/admin/operations");
  },

  async startAdminOperation(operation: string, options: { apply?: boolean; onlyEmpty?: boolean } = {}): Promise<AdminOperationStatus> {
    return request<AdminOperationStatus>("/admin/operations", {
      method: "POST",
      body: JSON.stringify({ operation, ...options }),
    });
  },

  async getAdminOperation(operationId: string): Promise<AdminOperationStatus> {
    return request<AdminOperationStatus>(`/admin/operations/${encodeURIComponent(operationId)}`);
  },

  async getAdminOperationLog(operationId: string): Promise<{ id: string; log: string }> {
    return request<{ id: string; log: string }>(`/admin/operations/${encodeURIComponent(operationId)}/log`);
  },

  async cancelAdminOperation(operationId: string): Promise<{ ok: boolean; operation_id: string }> {
    return request<{ ok: boolean; operation_id: string }>(`/admin/operations/${encodeURIComponent(operationId)}/cancel`, {
      method: "POST",
    });
  },

  async deleteAdminOperation(operationId: string): Promise<{ ok: boolean; operation_id: string }> {
    return request<{ ok: boolean; operation_id: string }>(`/admin/operations/${encodeURIComponent(operationId)}`, {
      method: "DELETE",
    });
  },
  ...
  // ==========================================
  // Watch Party (Teleparty) API
  // ==========================================
  async getRoomInfo(code: string): Promise<{
    exists: boolean;
    roomCode: string;
    code: string;
    media: {
  ...
  async createRoom(media: any): Promise<{
    roomCode: string;
    code: string;
    room: any;
  }> {
