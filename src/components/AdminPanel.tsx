...
interface AdminPanelProps {
  isOpen?: boolean;
  onClose?: () => void;
  onPlayDirect?: (streamResult: StreamResult) => void;
  onPlay?: (streamResult: StreamResult) => void;
  onPlayStream?: (streamResult: StreamResult) => void;
  [key: string]: any;
}
...
export const AdminPanel: React.FC<AdminPanelProps> = (props) => {
  ...
  const [activeTab, setActiveTab] = useState<'smart' | 'batch' | 'worker_tasks' | 'stream_tester' | 'library'>('smart');
  ...
  const [rateLimitDelay, setRateLimitDelay] = useState<number>(1500);
  ...
  const handleStartCrawler = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/v1/catalog/crawl', {
        ...
        body: JSON.stringify({
          url: crawlerUrl.trim(),
          max_pages: crawlerScope === 'full_catalog' ? 20 : crawlerPages,
                ...
                <div className="space-y-4">
                  <form onSubmit={handleStartCrawler} className="p-4 rounded-xl bg-zinc-900/60 border border-zinc-800 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                      ...
                      <div className="sm:col-span-4">
                        <label className="text-xs font-semibold text-zinc-300 block mb-1">
                          {crawlerScope === 'full_catalog' ? 'Límite Máximo de Páginas:' : 'Páginas a rastrear:'}
                        </label>
                        <input
                          type="number"
                          min={1}
                          max={crawlerScope === 'full_catalog' ? 50 : 20}
                          value={crawlerPages}
