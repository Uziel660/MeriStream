  ...
  return (
    ...
    >
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="auth-title" tabIndex={-1} className="auth-panel relative w-full max-w-md overflow-hidden rounded-2xl bg-zinc-950/95 border border-zinc-800/80 shadow-2xl shadow-amber-500/10 p-6 sm:p-8">
        ...
        {error && (
          <div role="alert" className="mb-4 flex items-center gap-2.5 p-3 text-xs text-red-400 bg-red-950/40 border border-red-800/60 rounded-xl">
        ...
        {successMsg && (
          <div role="status" aria-live="polite" className="mb-4 flex items-center gap-2.5 p-3 text-xs text-emerald-400 bg-emerald-950/40 border border-emerald-800/60 rounded-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {mode === "register" && (
            <div>
              ...
              <div className="auth-avatar-options flex items-center gap-3" role="group" aria-label="Color de perfil">
          ...
          >
            {isSubmitting ? (
              <><span className="inline-block h-4 w-4 border-2 border-black border-t-transparent rounded-full animate-spin" aria-hidden="true" /><span>Procesando…</span></>
            ) : mode === "login" ? (
              <>
