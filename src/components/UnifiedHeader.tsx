...
export const UnifiedHeader: React.FC<UnifiedHeaderProps> = ({ onSearchChange, activeFilter, onSelectCategory, searchQuery }) => {
  ...
  useEffect(() => {
    const outside = (e: PointerEvent) => { if (userMenuRef.current && !userMenuRef.current.contains(e.target as Node)) setIsUserMenuOpen(false); };
  ...
  useEffect(() => { if (mobileSearchOpen) requestAnimationFrame(() => searchInputRef.current?.focus()); }, [mobileSearchOpen]);

  const handleSelectTab = (id: string) => { setQuery(''); setMobileSearchOpen(false); onSearchChange(''); onSelectCategory(id); };
    ...
    <header id="main-unified-header" className={`site-header ${atTop ? 'is-at-top' : ''}`}>
      <div className="header-inner">
        <div className="header-top">
          ...
          <div className={`header-search ${mobileSearchOpen ? 'is-mobile-open' : ''}`} role="search">
            ...
            <button type="button" className="search-clear sm:hidden" onClick={() => setMobileSearchOpen(false)} aria-label="Cerrar búsqueda"><X size={17} /></button>
          </div>

          <div className="flex items-center justify-end gap-1">
            <button type="button" className="mobile-search-trigger ui-icon-button" onClick={() => setMobileSearchOpen(true)} aria-label="Abrir búsqueda" aria-expanded={mobileSearchOpen}><Search size={18} /></button>
              ...
              {isAuthenticated && user ? (
                <>
                  ...
                  <AnimatePresence>{isUserMenuOpen && (
                    <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ duration: .16 }} className="account-menu">
