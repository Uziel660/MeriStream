...
const VerificationPanel: React.FC = () => {
  ...
  const [adminOperationDefinitions, setAdminOperationDefinitions] = useState<AdminOperationDefinition[]>([]);
  const [adminOperations, setAdminOperations] = useState<AdminOperationStatus[]>([]);
  const [selectedAdminOperation, setSelectedAdminOperation] = useState("repair_catalog_ids");
  const [adminOperationApply, setAdminOperationApply] = useState(false);
  const [adminOperationOnlyEmpty, setAdminOperationOnlyEmpty] = useState(true);
  const [adminOperationBusy, setAdminOperationBusy] = useState(false);
  ...
  const loadAdminOperations = useCallback(async () => {
    try {
      const result = await api.getAdminOperations();
      setAdminOperationDefinitions(result.definitions || []);
      setAdminOperations(result.operations || []);
      if (result.definitions?.length > 0 && !result.definitions.some((item) => item.id === selectedAdminOperation)) {
        setSelectedAdminOperation(result.definitions[0].id);
      }
    } catch {
      // El resto de Verificación sigue funcionando aunque el catálogo de
      // operaciones no esté disponible durante un reinicio del servidor.
    }
  }, [selectedAdminOperation]);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void loadAdminOperations(), 3000);
    return () => window.clearInterval(timer);
  }, [loadAdminOperations]);
  ...
  const selectedAdminDefinition = adminOperationDefinitions.find((item) => item.id === selectedAdminOperation) || null;
  const activeAdminOperation = adminOperations.find((item) => item.operation === selectedAdminOperation && (item.state === "queued" || item.state === "running"));
  const latestAdminOperation = adminOperations[0] || null;

  const runAdminOperation = async () => {
    if (!selectedAdminDefinition || adminOperationBusy || activeAdminOperation) return;
    if (selectedAdminDefinition.mutating && adminOperationApply && !window.confirm(`¿Aplicar los cambios de «${selectedAdminDefinition.label}»?`)) return;
    setAdminOperationBusy(true);
    setActionFeedback(null);
    try {
      const operation = await api.startAdminOperation(selectedAdminOperation, {
        apply: adminOperationApply,
        onlyEmpty: adminOperationOnlyEmpty,
      });
      setAdminOperations((current) => [operation, ...current.filter((item) => item.id !== operation.id)]);
      setActionFeedback({ tone: "success", message: `${selectedAdminDefinition.label} iniciada.` });
    } catch (error) {
      setActionFeedback({ tone: "error", message: errorMessage(error) });
    } finally {
      setAdminOperationBusy(false);
      void loadAdminOperations();
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white">Operaciones manuales</h3>
            </div>
            <p className="mt-1 max-w-2xl text-[11px] text-zinc-400">
              Las tareas que antes se ejecutaban con comandos ahora se pueden lanzar, seguir y auditar desde aquí.
            </p>
          </div>
        ...
        {selectedAdminDefinition ? (
          <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
            <div className="space-y-2">
              <label className="block text-[10px] font-semibold uppercase tracking-wide text-zinc-500" htmlFor="admin-operation-select">
                Operación
              </label>
              <select
                id="admin-operation-select"
                value={selectedAdminOperation}
                onChange={(event) => {
                  setSelectedAdminOperation(event.target.value);
                  setAdminOperationApply(false);
                }}
                className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-xs text-zinc-200 outline-none focus:border-violet-400"
              >
                {adminOperationDefinitions.map((definition) => (
                  <option key={definition.id} value={definition.id}>{definition.label}</option>
                ))}
              </select>
              <p className="text-[11px] leading-relaxed text-zinc-500">{selectedAdminDefinition.description}</p>
              {selectedAdminDefinition.requiresApply && (
                <p className="text-[10px] text-amber-300">Esta operación escribe o crea tareas y requiere activar «Aplicar cambios».</p>
              )}
              <div className="flex flex-wrap items-center gap-4 pt-1">
                {selectedAdminDefinition.mutating && (
                  <label className="flex items-center gap-2 text-[11px] text-zinc-300">
                    <input
                      type="checkbox"
                      checked={adminOperationApply}
                      onChange={(event) => setAdminOperationApply(event.target.checked)}
                      className="h-3.5 w-3.5 accent-emerald-500"
                    />
                    Aplicar cambios
                  </label>
                )}
                {["repair_catalog_ids", "repair_tmdb_safe", "repair_anime_ids", "verify_catalog_ids"].includes(selectedAdminOperation) && (
                  <label className="flex items-center gap-2 text-[11px] text-zinc-300">
                    <input
                      type="checkbox"
                      checked={adminOperationOnlyEmpty}
                      onChange={(event) => setAdminOperationOnlyEmpty(event.target.checked)}
                      className="h-3.5 w-3.5 accent-violet-500"
                    />
                    Solo obras sin ningún ID
                  </label>
                )}
              </div>
            </div>
            <div className="flex items-end lg:min-w-48">
              <button
                type="button"
                onClick={() => void runAdminOperation()}
                disabled={adminOperationBusy || Boolean(activeAdminOperation) || Boolean(selectedAdminDefinition.requiresApply && !adminOperationApply)}
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {adminOperationBusy || activeAdminOperation ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
                Ejecutar operación
              </button>
            </div>
          </div>
        ) : (
          <p className="mt-3 text-[11px] text-zinc-500">Cargando operaciones disponibles…</p>
        )}

        {latestAdminOperation && (
          <div className="mt-4 rounded-lg border border-zinc-800 bg-zinc-950/50 p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold text-zinc-200">{latestAdminOperation.label}</p>
                <p className="mt-0.5 text-[10px] text-zinc-500">{formatDate(latestAdminOperation.updated_at)}</p>
              </div>
              <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                latestAdminOperation.state === "completed" ? "bg-sky-500/15 text-sky-300" :
                latestAdminOperation.state === "failed" ? "bg-red-500/15 text-red-300" :
                "bg-emerald-500/15 text-emerald-300"
              }`}>
                {latestAdminOperation.state === "completed" ? "COMPLETADA" : latestAdminOperation.state === "failed" ? "CON ERROR" : "EN CURSO"}
              </span>
            </div>
            {(latestAdminOperation.state === "queued" || latestAdminOperation.state === "running") && (
              <div className="mt-3 rounded-lg border border-violet-500/20 bg-violet-500/5 p-2.5">
                <div className="flex items-center justify-between gap-3 text-[10px] text-zinc-400">
                  <span>{latestAdminOperation.current_item || "Esperando la primera actualización de la operación…"}</span>
                  <strong className="shrink-0 text-violet-200">{typeof latestAdminOperation.progress_percent === "number" ? `${latestAdminOperation.progress_percent}%` : "En curso"}</strong>
                </div>
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-800">
                  <div className={`h-full rounded-full bg-violet-400 transition-all ${typeof latestAdminOperation.progress_percent === "number" ? "" : "w-1/3 animate-pulse"}`} style={typeof latestAdminOperation.progress_percent === "number" ? { width: `${latestAdminOperation.progress_percent}%` } : undefined} />
                </div>
              </div>
            )}
            {latestAdminOperation.output_tail && (
              <details className="mt-2">
                <summary className="cursor-pointer text-[10px] text-zinc-500 hover:text-zinc-300">Ver salida y reporte</summary>
                <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap break-words rounded bg-black/30 p-2 text-[10px] leading-relaxed text-zinc-400">{latestAdminOperation.output_tail}</pre>
              </details>
            )}
          </div>
        )}
