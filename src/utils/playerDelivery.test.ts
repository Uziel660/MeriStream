...
describe('playerDelivery — 8 pruebas obligatorias', () => {
  ...
  it('Chromecast usa sesión proxy aunque el navegador pueda reproducir el CDN directo', () => {
    const server = makeServer({ is_proxyable: true, delivery_mode: 'direct' });
...
describe('playerDelivery — helpers adicionales y estabilidad', () => {
  ...
  it('isUnresolvedCanonical detecta páginas de catálogo que no deben montarse en iframe', () => {
    ...
    expect(isUnresolvedCanonical('https://www.they.tube/nqd4tq7xml8j.html')).toBe(true);
    expect(isUnresolvedCanonical('https://gnula.nu/nuevo/player.php?id=abc123')).toBe(true);
    expect(isUnresolvedCanonical('https://animeav1.com/media/one-piece/1')).toBe(true);
