...
describe("subtitleTextToWebVtt", () => {
  ...
  it("does not render ASS alignment tags embedded in SRT/WebVTT dialogue", () => {
    expect(parseWebVttCues("1\n00:00:01,000 --> 00:00:02,500\n{an8}En ese lugar\\Nhabitado")[0]?.text).toBe(
      "En ese lugar\nhabitado",
    );
    expect(sanitizeSubtitleText("{\\an8}Texto {\\pos(10,20)}")).toBe("Texto ");
  });
});
