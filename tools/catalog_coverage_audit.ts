...
async function main(): Promise<void> {
  const items = await prisma.mediaItem.findMany({
    select: {
      ...
      kind: true,
      year: true,
      episodes: { select: { season_number: true, episode_number: true, links: { select: { source_site: true, source_status: true, link_type: true } } } },
    },
  });
  ...
  const report = {
    ...
    totals: { media_items: items.length, media_episodes: mediaEpisodes, episodes_without_links: episodesWithoutLinks, episodes_with_multiple_sites: episodesWithMultipleSites },
  ...
  const lines = [
    ...
    "",
    `MediaItems: ${items.length} · MediaEpisodes: ${mediaEpisodes} · Sin SourceLink: ${episodesWithoutLinks} · Con 2+ proveedores: ${episodesWithMultipleSites}`,
    "",
